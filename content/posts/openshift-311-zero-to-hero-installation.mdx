---
title: "Openshift 3.11 zero to hero installation"
publishedDate: "2018-11-08"
lastUpdatedDate: "2018-11-08"
tags:
  - technology
description: "Complete guide to install Openshift OKD 3.11 into VMs using libvirt and openshift-ansible playbook with custom DNS configuration."
status: published
---
...
- You want to expose your cloud inside a LAN (WiFi) not internet ✔️

- You don’t want to use minishift project or want to understand more about OKD installation process ✔️

- You have a ‘bulky’ laptop or Desktop PC for use as dedicated openshit cluster and want to work from another computer with development tools only ✔️

This is meant to be done _before_ you run the openshift-ansible playbook, but it can be adjusted if you have already installed openshift using openshift-ansible playbook.
...
For my current scenario, I’ll be using my dear old Alienware 18 🚀
...
For the bare-metal ‘bulky’ laptop I have choosen to use Fedora 29.
You can install a server centered OS like CentOS or Red Hat Enterprise Linux (requires active subscription).
...
Navigate through /etc/sysconfig/network-scripts
...
To enable guest request forwarding you need to enable the net.ipv4.ip_forward=1.
...
Open virt-manager application, then check if you’re currently have an virtual-network configured.

If you don’t, create a new one following the image below:

![virtual-network](/images/openshift-311-zero-to-hero-installation/c01.png)

![virtual-network](/images/openshift-311-zero-to-hero-installation/c02.png)

To keep this virtual-network active every boot, then execute:
...
![virtual-network](/images/openshift-311-zero-to-hero-installation/c03.png)
...
![iso](/images/openshift-311-zero-to-hero-installation/c04.png)

And finally check if your wireless interface is currently active with start mode set to onboot:

![wireless-interface](/images/openshift-311-zero-to-hero-installation/c05.png)
...
$ sudo mkdir /var/www/html/ks-cfg
...
After this, you should be able to grab the kickstart from the browser.
...
Your guest will start to install 💥 _automagically_ 💥 and after you will be prompted for root access in the console.
...
![guest](/images/openshift-311-zero-to-hero-installation/c06.png)

### 2.4 Check guest network configuration

In order to keep everything setup for the next steps, certify that network settings are well defined.
...
Access guest VM through ssh protocol:
...
Access guest through ssh protocol again:

$ ssh root@192.168.50.10

Execute the _openshift-ansible playbook_ from [gshipley installcentos scripts](https://github.com/gshipley/installcentos) installation script:
...
![openshift](/images/openshift-311-zero-to-hero-installation/c07.png)

Check your current version, accessing the following path:

https://console.arekkusu.io:8443/console/about

![openshift](/images/openshift-311-zero-to-hero-installation/c08.png)
...
will contact for DNS resolution. _Do not_ install it on any of the master nodes, since it will conflict with the Kubernetes DNS service.
...
On _all the nodes_, make sure that /etc/resolv.conf has the IP of the node running dnsmasq only. **Important:** on the node running
...
<p>PS. Only execute this step if you have more than one node running in the cluster topology.</p>

<h3>4.6 Configure ifcfg-eth0 file (all nodes)</h3>

<p>Make sure that in <code>/etc/sysconfig/network-scripts/ifcfg-eth0</code>, the variable <code>PEERDNS</code> is set to <code>no</code> so that <code>/etc/resolv.conf</code> doesn't get

overwritten on each reboot. Reboot the machines and check that <code>/etc/resolv.conf</code> hasn't been overwritten.</p>

<p>It will be like the following:</p>

<pre>
  <code>
    # Generated by parse-kickstart IPV6INIT="no" DHCP_HOSTNAME="localhost.localdomain" DNS2="8.8.4.4" DNS1="8.8.8.8"
    BOOTPROTO="static" GATEWAY=192.168.50.1 DEVICE="eth0" NETMASK=255.255.255.0 IPADDR=192.168.50.10 ONBOOT="yes"
    UUID=9x99x9x-99×9-9x99x-xxxx-99xx99x9x9xxx9x9 PEERDNS=no
  </code>
</pre>

<h3>4.7 Open DNS port and enable dnsmasq (main cluster)</h3>

<p>Finally, make sure port 53/udp is open on the dnsmasq node. We have to use iptables rules for this, even if you have <code>firewalld</code> installed.

Otherwise, the <code>openshift-ansible</code> playbook will disable and mask it and we will lose those rules. If you do have <code>firewalld</code>, let's mask it and

replace it with <code>iptables-services</code>:</p>
...
To do so, check the ![qemu](/images/openshift-311-zero-to-hero-installation/qemu) script and follow it’s orientations.
...
[https://www.stevenrombauts.be/2018/01/use-dnsmasq-instead-of-etc-hosts](https://www.stevenrombauts.be/2018/01/use-dnsmasq-instead-of-etc-hosts)

[https://blog.thesparktree.com/local-development-with-wildcard-dns](https://blog.thesparktree.com/local-development-with-wildcard-dns)

[https://www.sothawo.com/2018/04/adding-local-dns-resolution-for-a-custom-top-level-domain-on-macos-using-macports](https://www.sothawo.com/2018/04/adding-local-dns-resolution-for-a-custom-top-level-domain-on-macos-using-macports)

[https://medium.com/@narakuw/brew-install-dnsmasq-in-macos-sierra-26021c824be8](https://medium.com/@narakuw/brew-install-dnsmasq-in-macos-sierra-26021c824be8)

[https://gist.github.com/aelkz/01f2980daca2ac9a4d653a2936411bb2](https://gist.github.com/aelkz/01f2980daca2ac9a4d653a2936411bb2)

[https://gist.github.com/aelkz/cabfed5e8980c85d721cb9ef1ff804ee](https://gist.github.com/aelkz/cabfed5e8980c85d721cb9ef1ff804ee)
...
![openshift](/images/openshift-311-zero-to-hero-installation/c09.png)
...
It’s because your registry cannot be found inside cluster. I don’t know what may have did this, I had to add the following line in guest VM /etc/hosts file:

172.30.73.96   docker-registry.default.svc docker-registry.default.svc.cluster.local docker-registry-default.apps.arekkusu.io
where 172.30.73.96 is the docker registry cluster ip address obtained from:
...
![wildfly](/images/openshift-311-zero-to-hero-installation/c10.png)

Test your application, opening the web browser: http://myapp-demo.apps.arekkusu.io or via curl:
...
Test: ping grafana-openshift-monitoring.apps.arekkusu.io

Test: ping apiserver-kube-service-catalog.apps.arekkusu.io

Test: ping myapp-demo-internal.192.168.50.10.nip.io
...
Test: ping myapp-demo.apps.arekkusu.io

Voilá! If you have reached this point, you’re ready to go! 😎
