      ...
      <div className="container max-w-6xl pb-10">
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <article className="prose max-w-7xl dark:prose-invert hover:prose-a:text-accent-foreground prose-a:prose-headings:mb-3 prose-a:prose-headings:mt-8 prose-a:prose-headings:font-heading prose-a:prose-headings:font-bold prose-a:prose-headings:leading-tight prose-a:prose-headings:no-underline lg:mr-8 lg:max-w-2xl">
            <h1 className="mb-2 flex items-center gap-2 font-heading">
              {post.title}
              {post.isNew && <Image src="/new.png" alt="New" width={40} height={40} className="inline-block" />}
            </h1>
            {post.description && (
              <p className="mb-2 mt-0 text-xl text-slate-600 dark:text-slate-400">{post.description}</p>
            )}
            <hr className="my-4" />
            {post?.series && (
